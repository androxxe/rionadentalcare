	...
	<body>
		...
		<?php
			...
			if (isset($_POST['tambahin'])) {
				...
				$scekadakah			= $this->db->query("SELECT username FROM dokter WHERE username = '$username'");
				...
				if($hcekadakah > 0){
					...
					$wadafak 	= $this->db->query("INSERT INTO dokter (nama, alamat, tempat_lahir, tanggal_lahir, jenis_kelamin, no_telp, email, no_sip, no_str, tanggal_berlaku_sip, tanggal_berlaku_str, username, password, status) VALUE ('$nama','$alamat','$tempat_lahir','$tanggal_lahir','$jenis_kelamin','$no_telp','$email','$no_sip','$no_str','$tanggal_berlaku_sip','$tanggal_berlaku_str','$username','$password',$status)");
				...
				$scekadakah			= $this->db->query("SELECT username FROM dokter WHERE username = '$username' AND id_dokter <> '$id_dokter'");
				$hcekadakah			= $scekadakah->num_rows();
				if($hcekadakah > 0){
					...
					$wadafak 			= $this->db->query("UPDATE dokter SET nama = '$nama', alamat = '$alamat', tempat_lahir = '$tempat_lahir', tanggal_lahir = '$tanggal_lahir', jenis_kelamin = '$jenis_kelamin', no_telp = '$no_telp', email = '$email', no_sip = '$no_sip', no_str = '$no_str', tanggal_berlaku_sip = '$tanggal_berlaku_sip', tanggal_berlaku_str = '$tanggal_berlaku_str', username = '$username', status = '$status' WHERE id_dokter = '$id_dokter'");
				...
				$wadafak 	= $this->db->query("UPDATE dokter SET `password` = '" . $newpass . "' WHERE id_dokter = '" . $id_dokter . "'");
				...
				$id_dokter			= $this->input->post('id_dokter');
				$wadafak	= $this->db->delete('dokter', array('id_dokter' => $id_dokter));
				if ($wadafak) {
					echo "
					...
					<!-- Basic datatable -->
							...
							<div class="card">
								<div class="card-header bg-dark text-white header-elements-inline pt-1 pb-1">
									<h5 class="card-title">Master Data : <b class="text-warning">Dokter</b></h5>
...
-->
										...
										<tbody>
											<?php
												$sdata = $this->db->query("SELECT * FROM dokter ORDER BY nama");
												$nodata	= 1;
												...
												foreach ($sdata->result_array() as $ddata) {
													...
													if($ddata['status'] == 1){
														$statuse	= "<span class='badge badge-success'>AKTIF</span>";
													}else if($ddata['status'] == 0){
														$statuse	= "<span class='badge badge-danger'>TIDAK AKTIF</span>";
													}else{
														$statuse	= "<span class='badge bg-dark'>TIDAK DIKETAHUI</span>";
													}
													echo "
											<tr>
												<td class='text-center'>" . $nodata . "</td>
												<td>
													No. SIP : <b>" . $ddata['no_sip'] . "</b><br>
												...
												<td class='text-center'>
													...
													<div class='dropdown-menu dropdown-menu-right'>
														<a href='#' data-toggle='modal' data-target='#modalEdit' data-id='".$ddata['id_dokter']."' class='dropdown-item btedit'><i class='icon-pencil'></i> Edit Data</a>
														<a href='#' data-toggle='modal' data-target='#modalHapus' data-id='".$ddata['id_dokter']."' data-nama='".$ddata['nama']."' class='dropdown-item bthapus'><i class='icon-trash'></i> Hapus Data</a>
														<a href='#' data-toggle='modal' data-target='#modalEditPass' data-id='".$ddata['id_dokter']."' data-nama='".$ddata['nama']."' class='dropdown-item bteditpass'><i class='icon-lock'></i> Edit Password</a>
													</div>
												</td>
											</tr>
				...
				<div id="modalHapus" class="modal fade" tabindex="-1">
                    <div class="modal-dialog">
                        <div class="modal-content">
							<form method="post">
								<input type="hidden" name="id_dokter" id="id_dokterxx">
						...
						<div class="modal-content">
							...
							<div class="modal-body pb-0">
								<form method="post" class="formulir_tambah">
									...
									<div class="form-group">
										<label>No. SIP :</label>
										<input type="text" name="no_sip" class="form-control" placeholder="Nomor SIP" required>
									</div>
									<div class="form-group">
										<label>Tgl. Berlaku SIP :</label>
										<div class="form-group form-group-feedback form-group-feedback-left">
											<input type="text" class="form-control pickadate-wasem2" placeholder="Tanggal Berlaku SIP" name="tanggal_berlaku_sip" required>
											<div class="form-control-feedback form-control-feedback-sm">
												<i class="icon-calendar"></i>
											</div>
										</div>
						...
						<div class="modal-content">
							...
							<div class="modal-body pb-0">
								<form method="post" class="formulir_edit">
									...
									<div class="form-group">
										<label>No. SIP :</label>
										<input type="text" name="no_sip" class="form-control" id="no_sip" placeholder="Nomor SIP" required>
									</div>
									<div class="form-group">
										<label>Tgl. Berlaku SIP :</label>
										<div class="form-group form-group-feedback form-group-feedback-left">
											<input type="text" class="form-control pickadate-wasem2" placeholder="Tanggal Berlaku SIP" name="tanggal_berlaku_sip" id="tanggal_berlaku_sip" required>
											<div class="form-control-feedback form-control-feedback-sm">
												<i class="icon-calendar"></i>
											</div>
										</div>
				...
				<div id="modalEditPass" class="modal fade">
					<div class="modal-dialog">
						<div class="modal-content">
							<div class="modal-header bg-teal-600">
							<h4 class="modal-title">Edit Password Dokter : <b class="text-warning" id="namaxxx"></b></h4>
							<button type="button" class="close" data-dismiss="modal">&times;</button>
						</div>

							<div class="modal-body pb-0">
								<form method="post" class="formulir_editpass">
									<input type="hidden" name="id_dokter" id="id_dokter_editpass">
		...
		<script type="text/javascript">
			$(document).ready(function() {
				...
				$(document).on('click', '.btedit', function() {
					...
					$('#id_dokter_edit').val(id);
					$.ajax({
						method: 'post',
						//~ url: "<?= site_url('m/jkbyid') ?>",
						url: "<?= site_url('m/user_dokterbyid') ?>",
						data: {
							id: id
						},
						method: 'post',
						...
						success: function(data) {
							...
							$('#no_sip').val(data[0].no_sip);
							$('#no_str').val(data[0].no_str);
							$('#tanggal_berlaku_sip').val(data[0].tanggal_berlaku_sip);
							...
							var html2 = '<option value="">- Pilih Status -</option>';
							if(status2 == "1"){
								html2 += '<option value="1" selected>Aktif</option><option value="0">Tidak Aktif</option>';
							}else if(status2 == "0"){
								html2 += '<option value="1">Aktif</option><option value="0" selected>Tidak Aktif</option>';
							}else{
								html2 += '<option value="1">Aktif</option><option value="0">Tidak Aktif</option>';
							}
		...
		$('.formulir_tambah').validate({
            ...
            messages: {
				...
				no_sip:{
					required: 'No. SIP tidak boleh kosong'
				},
				no_str:{
					required: 'No. STR tidak boleh kosong'
				},
				tanggal_berlaku_sip:{
					required: 'Tanggal Berlaku SIP tidak boleh kosong'
				},
				tanggal_berlaku_str:{
