...
if ($ses_group == 1 || $ses_group == 2) {
	...
	<head>
		<title>Data Pengguna Operator - TV Riau</title>
		<?php
		...
		<?php
			...
			if (isset($_POST['tambahin'])) {
				...
				if($hcekadakah > 0){
					...
					$wadafak 	= $this->db->query("INSERT INTO tv_user (nm_user, id_jk, id_agama, alamat, hp, email, username, password, id_group) VALUE ('$nm_user','$id_jk','$id_agama','$alamat','$hp','$email','$username','$password','2')");
					...
					<!-- Basic datatable -->
							...
							<div class="card">
								<div class="card-header bg-dark text-white header-elements-inline pt-1 pb-1">
									<h5 class="card-title">Master Pengguna : <b class="text-warning">Operator</b></h5>
...
-->
								...
								<div class="card-body">
									...
									<table class="table datatable-reorder-realtime table-bordered table-hover table-stripted">
										...
										<tbody>
											<?php
												$sdata = $this->db->query("SELECT a.*, b.`nm_jk`, c.`nm_agama`, d.`nm_group` FROM tv_user a, tv_jk b, tv_agama c, tv_group d WHERE a.id_jk = b.`id_jk` AND a.`id_agama` = c.`id_agama` AND a.`id_group` = d.`id_group` AND a.id_group = '2' ORDER BY a.`nm_user`");
												$nodata	= 1;
