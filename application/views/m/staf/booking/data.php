					...
					";
						...
						<div class="p-3">
							<div class="bg-success p-3">
								<p>Nanti desain lebih rapi aja</p>
								<button class="btn-prev">Month sebelumnya</button>
								<button class="btn-now">Month saat ini</button>
								<button class="btn-next">Month selanjutnya</button>
		...
		<script type="text/javascript">
			...
			foreach ($sdata->result_array() as $ddata) { ?>
				calendar.createEvents([{
						id: '<?= $ddata['id_booking'] ?>',
						calendarId: 'cal_<?= $ddata['id_booking'] ?>',
						title: '<?= $ddata['nama_pasien'] ?>',
						body: '<?= $ddata['keterangan'] ?> - (<?= $ddata['status'] == 1 ? "Booking" : "Cancel" ?>)',
						start: new Date('<?= $ddata['tanggal_booking'] ?>T<?= $ddata['jam_booking'] ?>'),
						end: new Date('<?= $ddata['tanggal_booking'] ?>T<?= date('H:i', strtotime($ddata['tanggal_booking'] . ' ' . $ddata['jam_booking']) + 7200) ?>'),
						location: 'Ruangan',
						attendees: ['NAMA DOKTER NYAA'],
						category: 'time',
						state: 'Busy',
						isReadOnly: true,
						backgroundColor: <?= $ddata['status'] == 0 ? '"blue"' : '"green"' ?>,
						customStyle: {
							fontStyle: 'italic',
							fontSize: '15px',
							backgroundColor: 'red',
						},
						raw: JSON.parse('<?= json_encode($ddata) ?>'),
					}, // EventObject
				]);
			<?php } ?>

			// Fungsi untuk ke month sebelumnya
			$(".btn-prev").click(function() {
				calendar.prev();
			})

			// Fungsi untuk ke month selanjutnya
			$(".btn-next").click(function() {
				calendar.next();
			})

			// Fungsi untuk ke month saat ini
			$(".btn-now").click(function() {
				calendar.today();
			})

			// Fungsi yang dipanggil saat kita click event (Jadwal)
			// Disini aku contohkan dia redirect ke halaman Tambah

			// Contoh lain bisa juga untuk buka modal, dll, data nya ada di variable event
			// calendar.on('clickEvent', ({
			// 	event
			// }) => {
			// 	console.log('clickEvent', event)
			// 	let urlEdit = ''
			// 	if (event.raw.status == 0) {
			// 		urlEdit = ''
			// 	} else {
			// 		urlEdit = '-umum'
			// 	}

			// 	location.href = '<?= base_url("m/booking/tambah") ?>' + urlEdit + '/' + event.raw.id_booking

			// 	// m / booking / tambah " . $urledit . " / " . $ddata['id_booking']
			// 	// if ($ddata['status'] == 0) {
			// 	// 	// if($ddata['jenis'] == "terdaftar"){
			// 	// 	if ($ddata['jenis'] == "terdaftar") {
			// 	// 		$urledit = "";
			...
			// 	// 		$urledit = "-umum";
			// 	// 	}
			// 	// alert('asd',JSON.stringify(event))
			// });
